...
- 生育傾向マップで田を選ぶと、圃場の評価（S・A・B・C・D）と特徴を2〜3行で出す。100点満点:
  広さ30（500m²で0点〜1haで満点、対数）・生育のそろい25（田の中のばらつき＝上下1割を除いた幅 2〜15%）・
  形15（下記）・例年の旺盛さ10（8月ごろの NDVI の稲の年の平均の、同じセルの中での順位）・年ごとの安定10（年ごとの差 2〜8%）・
  深い所の少なさ10（前半に出遅れて後半に追いつく所の割合 0〜40%）。区切り S≧83・A≧73・B≧52・C≧35
  （県全体の稲の田 約12万枚で 5%・20%・50%・20%・5% になるように。獣害の減点の前）
  ...
  200m以内に森がある方位（16方位）、周り300mの田の割合を田ごとに作る。危なさ R = 0.55×近さ（0.5×f30+0.3×f100+0.2×f200、50%で満点）
  + 0.25×囲まれ（森の方位 1〜8）+ 0.20×孤立（周りの田 60%→0〜10%→1）で、R×20 点を引く
