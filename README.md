...
- 生育傾向マップで田を選ぶと、圃場の評価（S・A・B・C・D）と特徴を2〜3行で出す（いまは公開ページにも出す。地代・売買や個人の評価に使われるおそれがあるので、
  ...
  市町ごとの獣害の被害の多さは pipeline/wild_city.json（市町の資料から写した数字と出典）に書くと、評価に入る
  ランクの札にマウスを重ねる（スマホは押す）と、項目ごとの点・計算式・根拠が出る。選んだ田は「選択中圃場①〜⑳」（番号は線の色の順）
- 役所のサイトなど、作業用のコンテナからつながらない資料は、Actions の fetch-docs（URL を入れて Run workflow）で
  取ってきて、文字にしたものを fetch-docs ブランチに置ける
